...
@org.springframework.security.access.prepost.PreAuthorize(
        "@accessControl.has(authentication.name,'USER_MANAGE')"
)
public class PermissionController {

    private final PermissionAdminService service;

    public PermissionController(PermissionAdminService service) {
    ...
    public String index(
        ...
        long selected = roleId != null
                ? roleId
                : (roles.isEmpty() ? 0 : roles.getFirst().id());

        long selectedUser = userId != null
                ? userId
                : (users.isEmpty() ? 0 : users.getFirst().id());
        ...
        model.addAttribute(
                ...
                selectedUser == 0
                        ? java.util.List.of()
                        : service.userPermissions(selectedUser)
        );
    ...
    public String hierarchy(
            @PathVariable long id,
            @RequestParam int hierarchyOrder,
            Authentication auth,
            RedirectAttributes redirect
    ) {
        try {
